            ...
            <div className="mt-4">
                ...
                <Table className="text-sm text-slate-200 border-collapse [&_td]:align-top">
                  <TableHeader>
                    <TableRow className="bg-slate-900/70 text-xs uppercase tracking-[0.25em] text-slate-500">
                      ...
                      <TableHead className="border border-slate-800/60 bg-slate-950/60 text-slate-400">Help type</TableHead>
                      ...
                      <TableHead className="border border-slate-800/60 bg-slate-950/60 text-slate-400">Details</TableHead>
                    ...
                    {paginatedRequests.map((request) => (
                      ...
                      >
                        ...
                        <TableCell className="border border-slate-800/60 p-4 text-center">
                          <Badge className={`rounded-full border px-3 py-1 text-xs ${priorityStyles[request.priority].badge}`}>
                            {priorityStyles[request.priority].label}
                          </Badge>
                        </TableCell>
                        <TableCell className="border border-slate-800/60 p-4 text-center">
                          <Badge className={`rounded-full border px-3 py-1 text-xs ${statusStyles[request.status].badge}`}>
                        ...
                        <TableCell className="border border-slate-800/60 p-4">
                          <Button variant="outline" className="w-full rounded-xl border-slate-700 bg-slate-950/40 text-slate-100">
                            View
                          </Button>
                        </TableCell>
                      </TableRow>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-slate-300">Page {filteredRequests.length === 0 ? 0 : page + 1} / {filteredRequests.length === 0 ? 0 : pageCount}</span>
                  <Button
                    variant="outline"
                    size="sm"
        ...
        <DialogContent className="border border-slate-800 bg-slate-950 text-slate-100 sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              {activeRequest ? `Ticket ${activeRequest.id} · ${activeRequest.submittedAt}` : "Select a row to review details."}
            </DialogDescription>
          </DialogHeader>
          {activeRequest ? (
            <div className="space-y-4">
              <div className="flex flex-wrap gap-3">
                <Badge className={`rounded-full border px-3 py-1 text-xs ${priorityStyles[activeRequest.priority].badge}`}>
