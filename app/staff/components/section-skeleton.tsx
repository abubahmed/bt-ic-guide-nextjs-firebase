      ...
      <div className="mt-6 space-y-5">
        {blocks.map((block, index) => (
          <div
            ...
            className="rounded-2xl border border-slate-800/60 bg-slate-950/45 p-4">
            ...
            {block.contentHeight && (
              <div
                className={cn(
                  "mt-4 rounded-2xl bg-slate-900/60",
                  block.contentHeight,
      ...
      {footerItems > 0 && (
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {Array.from({ length: footerItems }).map((_, idx) => (
            <div
              key={`skeleton-footer-${idx}`}
              className="h-11 rounded-2xl bg-slate-900/80"
            />
          ))}
        </div>
      )}
