          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <div className="space-y-4 rounded-[28px] border border-slate-800/70 bg-slate-950/40 p-5">
              <div className="flex items-center gap-3 text-[0.65rem] uppercase tracking-[0.35em] text-slate-500">
                ...
                <span>Active map metadata</span>
              </div>
              {liveMeta ? (
                <>
                  <div>
                    <p className="text-lg font-semibold text-white">{liveMeta.name}</p>
                    <p className="text-sm text-slate-400">
                      {liveMeta.size} · {liveMeta.updated}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-slate-800/60 bg-slate-950/40 p-4 text-sm text-slate-400">
                    <p>
                      Latest file is synced to volunteer tablets, kiosks, QR signage, and the attendee microsite. Download
                      above if you need to annotate before the next upload.
                    </p>
                  </div>
                </>
              ) : (
                <div className="rounded-2xl border border-rose-500/40 bg-rose-500/10 p-4 text-sm text-rose-100">
                  No map published. Upload a file on the left to restore signage previews across the network.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-base text-slate-400">
                  The panel below mirrors the published asset in the staff portal. Delete to clear signage or stage a new
                  upload above.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 rounded-2xl border border-slate-800/70 bg-slate-950/40 px-4 py-3 text-[0.65rem] uppercase tracking-[0.35em] text-slate-500">
              <span>Status</span>
              <span className="text-sky-300">{liveMap ? "active" : "offline"}</span>
            </div>
          </div>
          <div className="mt-6 grid gap-6 lg:grid-cols-[3fr_1fr]">
            ...
            <div className="space-y-4 rounded-[28px] border border-slate-800/70 bg-slate-950/40 p-5">
              <div className="rounded-2xl border border-slate-800/60 bg-slate-950/60 p-4 text-sm text-slate-400">
                <p>
                  Need to rotate or clear signage temporarily? Delete the map here. The attendee QR experience will hide
                  the tile until a new file is staged.
                </p>
              </div>
              <Button
                variant="outline"
