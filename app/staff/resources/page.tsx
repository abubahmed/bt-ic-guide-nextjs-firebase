...
import { seededResources, visibilityStyles, sourceStyles } from "./data";
...
export default function StaffResourcesPage() {
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((entry) => {
      ...
      if (normalizedSearch.length > 0) {
        const haystack = `${entry.title} ${entry.owner}`.toLowerCase();
  ...
  const handleSubmit = () => {
    ...
    const referenceLabel = uploadMode === "link" ? formLink.trim() : formFile?.name ?? "Attachment";
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10 lg:px-0">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-2 text-base text-slate-400">
                  Upload reference materials and forms to the system. Ensure it matches the required format and headers.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="rounded-[32px] border border-slate-700 bg-slate-800/70 p-6 shadow-[0px_30px_80px_rgba(2,6,23,0.3)] lg:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-3">
              <div>
                <h2 className="text-3xl font-semibold text-white">Resource directory</h2>
                <p className="mt-2 text-base text-slate-400">
                  Filter by audience or source type, search across titles, and delete select resources.
                </p>
              </div>
            </div>
          </div>
                       ...
                       <TableRow key={entry.id} className="border border-slate-700/60">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Badge
                              className={`rounded-full px-3 py-1 text-[0.65rem] ${
                                visibilityStyles[entry.visibility].badge
                              }`}>
                        ...
                        <TableCell className="border border-slate-700/60 p-3">
                          <div className="flex flex-wrap gap-2">
                            <Badge
                              className={`rounded-full px-3 py-1 text-[0.65rem] ${
                                sourceStyles[entry.sourceType].badge
                              }`}>
